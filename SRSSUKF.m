...
% This class implements a square-root formulation of the Spherical Simplex Unscented Kalman Filter (SRSSUKF) for nonlinear state estimation problems.
% It is designed for systems with nonlinear process and/or measurement models and supports state representations that may include unit quaternions and angles.
% To use this filter:
...
classdef SRSSUKF < handle
    methods
        function obj = SRSSUKF(w0, xDim, wDim, xIdxAngle, xIdxQuaternion, cyDim, cvDim)
            %SRSSUKF.SRSSUKF Construct a Square-Root Spherical Simplex Unscented Kalman Filter object.
            % 
            % PARAMETERS
            % w0             ... Scalar weighting factor for 0-th sigma-point, range: [0;1).
            % xDim           ... Dimension of the actual state vector x.
            % wDim           ... Dimension of the process noise vector w (or zero for additive SRSSUKF). The state vector is augmented by the process noise vector.
            ...
            % cvDim          ... Cell array containing v-dimensions for all measurement noise vectors. The state vector is augmented by all measurement noise vectors.
        ...
        function state = Predict(obj, funcProc, Ts, u, sqrtQ, optArg)
            ...
            % Ts       ... Discrete sampletime in seconds (is forwarded to user-defined funcProc).
        ...
        function [state, innovation] = Update(obj, funcObsv, sensorIndex, y, idxAngle, sqrtR, optArg)
            ...
            % sensorIndex ... The one-based index of the sensor. The dimensions of observation y and noise v are given by the cell arrays cyDim and cvDim that have been
            %                 set by the constructor, respectively. sensorIndex is used to obtain the dimension of the measurement and the corresponding noise.
            ...
            sensorIndex = int32(sensorIndex);
            assert(~isempty(sensorIndex));
            assert((sensorIndex > 0) && (sensorIndex <= obj.numObsv));
            yDim = obj.infoObsv(1,sensorIndex);
            vIndexOffset = obj.infoObsv(2,sensorIndex);
            vDim = obj.infoObsv(3,sensorIndex);
        ...
        function x = GetInternalState(obj)
            %SRSSUKF.GetInternalState Get the current internal state vector.
            % 
            % RETURN
            % x ... Nx1 vector of the current internal state estimation with N being the dimension of the augmented state.
            x = obj.x;
        end
        ...
        function S = GetInternalSqrtCovariance(obj)
            %SRSSUKF.GetInternalSqrtCovariance Get the internal square-root covariance matrix.
            % 
            % RETURN
            % S ... NxN square-root state covariance matrix with N being the dimension of the augmented state. Note that quaternion-components are replaced
            %       by corresponding orientation-vector components.
            S = obj.S;
        end
        ...
        function SetInternalState(obj, x)
            %SRSSUKF.SetInternalState Override the internal state vector.
            % 
            % PARAMETER
        ...
        function SetSqrtR(obj, sensorIndex, sqrtR)
            %SRSSUKF.SetSqrtR Set square-root of observation covariance R.
            % 
            % PARAMETERS
            % sensorIndex   ... The one-based index of the sensor. The dimensions of observation y and noise v are given by the cell arrays cyDim and cvDim that have been set by
            %                   the constructor, respectively. sensorIndex is used to obtain the correct measurement noise dimension.
    ...
    properties(Access = protected)
        ...
        nxS           % Dimension of state vector (x) with respect to uncertainty S.
